(() => {
  ...
  const state = { nodes: new Map(), roots: [], files: [], activeCategory: "全部" };
  ...
  function buildTree(entries) {
    ...
    for (const e of dirEntries) {
      ...
      const node = { type: "dir", name: parts.at(-1), path: e.path, children: [], file_count: 0, dir_count: 0 };
      state.nodes.set(e.path, node);
    }
  ...
  function fileRow(node, showPath = false) {
    ...
    const b = document.createElement("div"); b.className = "item-body";
    const t = document.createElement("div"); t.className = "item-title"; t.textContent = node.name;
    const s = document.createElement("div"); s.className = "item-sub"; s.textContent = showPath ? node.path : size(node.size); b.append(t,s);
  ...
  function crumbs(path) {
    ...
    const home = document.createElement("button"); home.className = "crumb"; home.textContent = "首页"; home.onclick = () => go(""); breadcrumbs.append(home);
    if (!path) return;
  ...
  function home() {
    ...
    content.append(head(state.activeCategory === "全部" ? "全部课程" : state.activeCategory, `${filtered.length} 门课程 · 每张卡片都可直接打开或一键下载`));
  ...
  function directory(node) {
    ...
    const titleRow = head(node.title || node.name, `${node.file_count} 个文件 · ${node.dir_count} 个子目录`);
    if (node.is_course) {
      const action = document.createElement("a"); action.className = "btn primary section-download"; action.textContent = "一键下载整门课程"; action.href = folderDownloadUrl(node.path); action.target = "_blank"; action.rel = "noopener"; titleRow.append(action);
    }
  ...
  function searchResults(q) {
    ...
    if (files.length) { content.append(head("文件匹配", files.length > 100 ? `显示前 100 个，共 ${files.length} 个` : "可直接下载")); const list = document.createElement("div"); list.className = "list"; files.slice(0,100).forEach(x => list.append(fileRow(x,true))); content.append(list); }
  ...
  function matchScore(text, q) {
    const t = key(text), k = key(q); if (!t.includes(k)) return -1;
    if (t === k) return 100; if (t.startsWith(k)) return 70; return 40 - Math.min(20, t.indexOf(k));
  }

  function renderSuggestions(q) {
    const value = q.trim();
    if (!value || !state.roots.length) return hideSuggestions();
    const courseHits = state.roots
      .map((x) => ({ node: x, score: Math.max(matchScore(x.title, value), matchScore(x.code, value), matchScore(x.name, value)) }))
      .filter((x) => x.score >= 0).sort((a,b) => b.score - a.score).slice(0, 5);
    const fileHits = state.files
      .map((x) => ({ node: x, score: Math.max(matchScore(x.name, value), matchScore(x.path, value)) }))
      .filter((x) => x.score >= 0).sort((a,b) => b.score - a.score).slice(0, 3);
    suggestions.replaceChildren();
    [...courseHits.map((x) => ({...x, kind:"course"})), ...fileHits.map((x) => ({...x, kind:"file"}))].forEach((hit) => {
      const row = document.createElement("button"); row.type = "button"; row.className = "suggestion-row";
      const i = document.createElement("span"); i.className = "suggestion-icon"; i.textContent = hit.kind === "course" ? "🎓" : icon(hit.node.name);
      const body = document.createElement("span"); body.className = "suggestion-body";
      const title = document.createElement("strong"); title.textContent = hit.kind === "course" ? (hit.node.title || hit.node.name) : hit.node.name;
      const sub = document.createElement("small"); sub.textContent = hit.kind === "course" ? `${hit.node.code || hit.node.category} · ${hit.node.file_count} 个文件` : hit.node.path;
      const tag = document.createElement("span"); tag.className = "suggestion-tag"; tag.textContent = hit.kind === "course" ? "课程" : "文件";
      body.append(title, sub); row.append(i, body, tag);
      row.onclick = () => {
        if (hit.kind === "course") { searchInput.value = ""; go(hit.node.path); }
        else { hideSuggestions(); window.open(viewUrl(hit.node.path), "_blank", "noopener"); }
      };
      suggestions.append(row);
    });
    if (!suggestions.children.length) return hideSuggestions();
  ...
  async function init() {
    try {
      ...
      buildTree(data.tree || []);
      statusLine.textContent = `${state.roots.filter(x=>x.is_course).length} 门课程 · ${state.files.length} 个文件 · 中文联想搜索已启用${data.truncated ? " · 部分超大目录可能被截断" : ""}`;
      render();
    } catch (err) {
      console.error(err);
      statusLine.textContent = "目录索引加载失败";
      content.innerHTML = '<div class="empty-card">暂时无法读取 GitHub 目录，请稍后刷新页面重试。</div>';
    }
  }

  searchInput.addEventListener("input", () => { renderSuggestions(searchInput.value); render(); });
